...
-- Tabela principal de chamados (atualizada)
CREATE TABLE IF NOT EXISTS chamados (
    ...
    atribuido_para VARCHAR(255), -- ID do usuário do time de projetos
    atribuido_nome VARCHAR(255), -- Nome do responsável
    atribuido_em TIMESTAMP,
    time_responsavel VARCHAR(100) DEFAULT 'projetos', -- Sempre 'projetos' por padrão
    
    -- SLA e prazos
    prazo_resposta TIMESTAMP, -- Prazo para primeira resposta
    prazo_resolucao TIMESTAMP, -- Prazo para resolução
    tempo_resposta_minutos INTEGER, -- Tempo real de primeira resposta
    tempo_resolucao_minutos INTEGER, -- Tempo real de resolução
    
    -- Anexos e mídia
    imagem_url TEXT,
    anexos JSONB DEFAULT '[]', -- Array de URLs de anexos
    
    -- Tags e metadados
    tags TEXT[], -- Array de tags
    ambiente VARCHAR(50), -- produção, desenvolvimento, homologação
    ...
    -- Feedback
    satisfacao INTEGER CHECK (satisfacao >= 1 AND satisfacao <= 5), -- 1-5 estrelas
    feedback TEXT,
...
CREATE TABLE IF NOT EXISTS chamados_comentarios (
    ...
    tipo VARCHAR(20) DEFAULT 'comentario', -- comentario, resolucao, reabertura, transferencia
    anexos JSONB DEFAULT '[]',
    interno BOOLEAN DEFAULT false, -- Se true, só o time vê
    created_at TIMESTAMP DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS chamados_notificacoes (
    ...
    chamado_id UUID NOT NULL,
    usuario_id VARCHAR(255) NOT NULL,
    tipo VARCHAR(50) NOT NULL, -- novo_chamado, atribuicao, comentario, resolucao, etc
    titulo VARCHAR(255) NOT NULL,
    mensagem TEXT,
...
CREATE TABLE IF NOT EXISTS chamados_sla (
    ...
    tempo_resposta_horas INTEGER NOT NULL, -- Tempo para primeira resposta
    tempo_resolucao_horas INTEGER NOT NULL, -- Tempo para resolução
    ativo BOOLEAN DEFAULT true,
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
...
-- Política: Usuários podem ver seus próprios chamados
CREATE POLICY "Usuários podem ver seus chamados" ON chamados
    FOR SELECT
    USING (auth.uid()::text = usuario_id OR auth.uid()::text = atribuido_para);

-- Política: Usuários podem criar chamados
CREATE POLICY "Usuários podem criar chamados" ON chamados
    FOR INSERT
    WITH CHECK (auth.uid()::text = usuario_id);

-- Política: Time de projetos pode ver todos os chamados
CREATE POLICY "Time de projetos pode ver todos" ON chamados
    ...
    USING (
        ...
        EXISTS (
            ...
            AND (raw_user_meta_data->>'departamento' = 'projetos' OR raw_user_meta_data->>'perfil' = 'admin')
        )
    );

-- Política: Time de projetos pode atualizar chamados atribuídos
...
-- Comentários: Usuários podem ver comentários de seus chamados
CREATE POLICY "Usuários podem ver comentários" ON chamados_comentarios
    ...
    USING (
        EXISTS (
            ...
            AND (chamados.usuario_id = auth.uid()::text OR chamados.atribuido_para = auth.uid()::text)
        ) OR
        NOT interno
    );

-- Comentários: Usuários podem criar comentários em seus chamados
CREATE POLICY "Usuários podem criar comentários" ON chamados_comentarios
    ...
    WITH CHECK (
        ...
        auth.uid()::text = chamados_comentarios.usuario_id
    );

-- Notificações: Usuários podem ver suas notificações
